...
export const articles: Record<Locale, Record<'how-to' | 'features' | 'about', Article>> = {
  ko: {
    'how-to': {
      ...
      sections: [
        { title: '01. 머물고 싶은 공간 고르기', image: guide + 'themes.png', alt: '햇살 온실과 비 오는 밤의 카페를 고르는 테마 페이지', caption: '사용 가능한 세트를 선택한 뒤 ‘이 세트로 시작’을 누릅니다.', paragraphs: ['테마 페이지에서 햇살 온실 또는 비 오는 밤의 카페를 선택하세요. 오른쪽 선택 영역에서 장면을 확인한 뒤 작업실을 열 수 있습니다. 휴대폰에서는 세트 목록 아래에 선택 영역이 나타납니다.', '현재 두 세트를 이용할 수 있으며 나머지는 준비 중입니다. 출시 세트는 난이도 구분 없이 개별 배치 가능한 상세 스티커 75개 이상을 목표로 제작합니다. 현재 공개된 세트는 기존 기능 체험용 구성입니다.'] },
        { title: '02. 스티커를 놓고 움직이기', image: guide + 'studio.png', alt: '온실에 스티커를 배치한 작업실과 오른쪽 스티커 서랍', caption: '서랍에서 스티커를 누르면 장면에 추가됩니다. 배치한 스티커를 끌어 옮겨보세요.', paragraphs: ['스티커 서랍의 가구·식물·소품 카테고리에서 원하는 것을 고르세요. 클릭하거나 탭하면 장면에 추가됩니다. PC에서는 서랍에서 장면으로 드래그해 놓을 수도 있습니다.', '장면에 놓은 스티커는 서랍에서 사라지고, 작품에서 삭제하면 원래 목록에 다시 나타납니다. 추가한 스티커를 누른 채 움직여 위치를 바꿀 수 있습니다. 휴대폰에서는 장면 아래 서랍을 이용합니다.'] },
        { title: '03. 회전·크기·겹치는 순서 조절하기', image: guide + 'rotate.png', alt: '작업실 아래에 고정된 회전 버튼, 각도, 크기 슬라이더와 레이어 도구', caption: '선택 도구는 장면 밖에 고정되어 스티커가 가장자리에 있어도 접근할 수 있습니다.', paragraphs: ['스티커를 선택하면 아래에 편집 도구가 나타납니다. −15°와 +15° 버튼으로 방향을 바꾸고 크기 슬라이더로 확대·축소하세요. 현재 각도를 보면서 같은 방향으로 정렬할 수 있습니다.', '앞으로·뒤로 또는 맨 앞·맨 뒤를 이용하면 화분을 책장 앞에 두는 것처럼 겹치는 순서를 바꿀 수 있습니다. 좌우 반전과 삭제도 이 도구에서 사용할 수 있습니다. 위치가 마음에 들면 잠가 두고, 다시 수정하려면 잠금을 해제하세요.', '실수했다면 상단의 실행 취소를 누르세요. PC에서는 Ctrl/Cmd+Z, 다시 실행은 Ctrl/Cmd+Shift+Z를 사용할 수 있습니다. 삭제를 취소하면 스티커는 작품으로 돌아오고 목록에서 다시 사라집니다.'] },
        ...
        { title: '자주 궁금한 점', paragraphs: ['가입이 필요한가요? 현재 꾸미기와 PNG 저장에는 계정이 필요하지 않습니다. 작품은 기본적으로 사용자 기기의 브라우저에 보관됩니다.', '감상 모드는 무엇인가요? 편집 도구를 숨기고 장면을 보는 모드입니다. 카페의 일부 스티커는 김이나 빛 등의 시각 효과를 보여줍니다. PNG로 저장하면 움직임은 포함되지 않습니다.', '난이도를 고르나요? 별도 난이도 선택은 없습니다. 모든 출시 세트를 세밀한 개별 오브젝트 중심으로 구성하고 세트당 최소 75개 이상의 스티커를 제공할 계획입니다.'] },
      ],
    },
    features: {
      ...
      sections: [
        ...
        { title: '이동부터 회전, 레이어까지 손에 익는 편집', image: guide + 'rotate.png', alt: '회전·크기·레이어를 조절하는 스티커 편집 도구', paragraphs: ['스티커를 움직이고 크기를 바꾸고 15도씩 회전할 수 있습니다. 좌우 반전과 레이어 순서로 사물의 방향과 앞뒤 관계를 표현하세요. 사용한 스티커는 목록에서 빠지고 삭제하면 다시 돌아옵니다.', '실행 취소와 다시 실행은 작품과 스티커 목록을 함께 복원합니다. 잠금 기능은 완성한 가구의 위치를 고정할 때 유용합니다. 편집 도구는 장면과 분리된 화면 안에 표시됩니다.'] },
        ...
        { title: '준비 중: 75개 이상의 상세 스티커', paragraphs: ['난이도를 나누지 않고 모든 출시 세트를 세밀한 개별 오브젝트 중심으로 구성합니다. 가구와 식물뿐 아니라 책, 도구, 작은 장식까지 직접 골라 배치하는 방향입니다.', '각 세트는 개별 배치 가능한 스티커를 최소 75개 이상 제공합니다. 현재 두 세트는 기존 기능 체험용이며, 완성 콘텐츠의 최대 수량은 실제 기기 성능을 확인한 뒤 결정합니다.'] },
  ...
  en: {
    'how-to': {
      ...
      sections: [
        { title: '01. Choose a space', image: guide + 'themes.png', alt: 'Theme selection with greenhouse and café sets', caption: 'Select an available set, then choose Start this set.', paragraphs: ['Open Themes and choose the Sunlit Greenhouse or Rainy Night Café. The selection panel appears beside the cards on desktop and below them on mobile.', 'Two preview sets are currently available. Release sets will have no difficulty tiers and will include at least 75 individually placeable detailed stickers.'] },
        { title: '02. Place and move stickers', image: guide + 'studio.png', alt: 'Greenhouse studio with placed stickers and a sticker drawer', caption: 'Tap a sticker in the drawer to add it, then drag it in the scene.', paragraphs: ['Browse furniture, plants and smaller objects in the drawer. Click or tap to add a sticker. On desktop, you can also drag it from the drawer into the scene.', 'A placed sticker leaves the drawer and returns when you delete it from the scene. Hold a placed sticker to move it. On mobile, the drawer sits below the scene.'] },
        { title: '03. Rotate, resize and change layers', image: guide + 'rotate.png', alt: 'Fixed rotation buttons, angle display, size slider and layer controls', caption: 'The controls stay outside the artwork, including when stickers approach its edges.', paragraphs: ['Select a sticker to show the editing controls below the artwork. Use −15° or +15° to rotate it and the size slider to resize it. The angle readout helps you align objects.', 'Move a sticker forward or backward, flip it or delete it. Lock a finished placement to prevent accidental movement and unlock it to edit again.', 'Undo and redo restore both the scene and the available sticker list. On desktop, use Ctrl/Cmd+Z and Ctrl/Cmd+Shift+Z.'] },
    ...
    features: {
      ...
      sections: [
        ...
        { title: 'Editing that encourages experiments', image: guide + 'rotate.png', alt: 'Sticker rotation and layer controls', paragraphs: ['Move and resize stickers, rotate in 15-degree steps, flip them, and change which ones appear in front. A used sticker leaves the drawer and returns when deleted.', 'Undo and redo restore both the scene and the drawer. Lock a finished object while you work on the rest of your scene.'] },
        ...
        { title: 'In preparation: 75+ detailed stickers', paragraphs: ['Release sets will not use difficulty tiers. Each collection will focus on detailed individual objects, from furniture and plants to books, tools and small decorations.', 'Each set will include at least 75 individually placeable stickers. The current two sets are functional previews; the maximum final count will follow device performance testing.'] },
