...
export function StickerTray({ isOpen, onToggleOpen, onAddSticker, assets, locale = 'ko' }: Props) {
 ...
 return <aside className={'sticker-drawer ' + (isOpen ? 'open' : 'closed')} aria-label={locale === 'ko' ? '스티커 서랍' : 'Sticker drawer'}>
   ...
   <div className="drawer-grid">{filtered.length === 0 ? <p className="drawer-empty" role="status">{locale === 'ko' ? '이 목록의 스티커를 모두 사용했어요. 작품에서 삭제하면 다시 나타납니다.' : 'You used every sticker in this list. Delete one from the scene to bring it back.'}</p> : filtered.map(asset => { const Component = STICKER_COMPONENTS[asset.assetId]; return <button className="sticker-card" key={asset.assetId} onClick={() => onAddSticker(asset.assetId)} draggable onDragStart={e => {e.dataTransfer.setData('application/diorama-asset-id',asset.assetId);e.dataTransfer.effectAllowed='copy';}} aria-label={asset.name[locale]}>
